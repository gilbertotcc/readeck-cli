...
def test_get_instance_info_parses_stable_release() -> None:
    ...
    info = get_instance_info(base_url=BASE_URL, api_token=API_TOKEN, transport=httpx.MockTransport(handler))
...
def test_get_instance_info_wraps_client_errors() -> None:
    ...
    with pytest.raises(CommandError):
        get_instance_info(base_url=BASE_URL, api_token=API_TOKEN, transport=httpx.MockTransport(handler))


def test_get_instance_info_wraps_invalid_config() -> None:
    with pytest.raises(CommandError):
        get_instance_info(base_url="", api_token=API_TOKEN)
