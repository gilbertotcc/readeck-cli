...
class OauthClientCreate(BaseModel):
    ...
    OauthClientCreate
    """ # noqa: E501
    client_name: Annotated[str, Field(strict=True, max_length=128)] = Field(description="The client's name")
    client_uri: Annotated[str, Field(strict=True, max_length=256)] = Field(description="An URL where client information can be found. HTTPS only.  The URL must resolve to a non local and non private IP address. ")
    logo_uri: Optional[Annotated[str, Field(strict=True, max_length=8192)]] = Field(default=None, description="An URL for the client's logo.  Only data URI of a base64 PNG encoded image is allowed. ")
    software_id: Annotated[str, Field(strict=True, max_length=128)] = Field(description="A unique identifier for the application, for example a UUID")
    software_version: Annotated[str, Field(strict=True, max_length=64)] = Field(description="Version of the client")
    redirect_uris: Optional[List[Annotated[str, Field(strict=True, max_length=256)]]] = Field(default=None, description="A list of possible redirection URIs. URIs must be one of:  - any https URI - http URI for loopback IP address (`127.0.0.0/8` or `[::1]`) - any other app link scheme (ie `my-app.org:/callback`)  This field is required when `grant_types` contains `authorization_code`. ")
    token_endpoint_auth_method: Optional[StrictStr] = Field(default='none', description="Client supported auth method")
    grant_types: Optional[List[StrictStr]] = Field(default=None, description="Client supported grant types")
    response_types: Optional[List[StrictStr]] = Field(default=None, description="Client supported response type")
    __properties: ClassVar[List[str]] = ["client_name", "client_uri", "logo_uri", "software_id", "software_version", "redirect_uris", "token_endpoint_auth_method", "grant_types", "response_types"]

    @field_validator('logo_uri', mode="before")
    def logo_uri_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if isinstance(value, str) and not re.match(r"^data:image\/png;base64,[A-Za-z0-9+\/]+$", value):
            raise ValueError(r"must validate the regular expression /^data:image\/png;base64,[A-Za-z0-9+\/]+$/")
        return value
    ...
    def grant_types_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['authorization_code', 'urn:ietf:params:oauth:grant-type:device_code']):
                raise ValueError("each list item must be one of ('authorization_code', 'urn:ietf:params:oauth:grant-type:device_code')")
        return value

    @field_validator('response_types')
    def response_types_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['code']):
                raise ValueError("each list item must be one of ('code')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OauthClientCreate from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OauthClientCreate from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "token_endpoint_auth_method": obj.get("token_endpoint_auth_method") if obj.get("token_endpoint_auth_method") is not None else 'none',
